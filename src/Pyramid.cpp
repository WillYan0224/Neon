	...
	TestObject(gfx, rng, adist, ddist, odist, rdist)
{
	...
	if (!IsStaticInitialized())
	{
		...
		auto pvs = std::make_unique<VertexShader>(gfx, L"src\\Shaders\\BlendedPhongVS.cso");
		auto pvsbc = pvs->GetByteCode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(gfx, L"src\\Shaders\\BlendedPhongPS.cso"));
