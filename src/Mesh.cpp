...
Mesh::Mesh(Graphics& gfx, std::vector<std::unique_ptr<Bind::Bindable>> bindPtrs)
{
	...
	AddBind(std::make_unique<Bind::TransformCB>(gfx, *this));
}
...
std::unique_ptr<Mesh> Model::ParseMesh(Graphics& gfx, const aiMesh& mesh)
{
	...
	auto pvs = std::make_unique<Bind::VertexShader>(gfx, L"src\\Shaders\\PhongVS.cso");
	auto pvsbc = pvs->GetByteCode();
	bindablePtrs.push_back(std::move(pvs));

	bindablePtrs.push_back(std::make_unique<Bind::PixelShader>(gfx, L"src\\Shaders\\PhongPS.cso"));
